...
for (i in seq_len(nrow(departements_L93))) {
  ...
  if (depts[n] != 0) {
    departements_L93$taux_acc_grave[i] <- depts_grave[n] / depts[n]
  }
  else {
...
for (i in seq_len(nrow(region_L93))) {
  ...
  if (reg[as.numeric(region_L93$CODE_REG[i])] != 0) {
    region_L93$taux_acc_grave[i] <- reg_grave[as.numeric(region_L93$CODE_REG[i])] / reg[as.numeric(region_L93$CODE_REG[i])]
  }
  else {
